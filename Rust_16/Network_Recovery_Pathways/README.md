...
1. Use binary search on <code>ans</code>.
2. Check if a particular <code>ans</code> is possible by including only the edges with weights ≥ <code>mid</code> (the current binary‐search pivot).
3. Implement the check function using either <code>Dijkstra</code> or DP (via topological sorting, since the graph is a DAG).
