...
1. Precompute, for each cell and each move (right/down), where you actually land if there’s a mirror next—store these "jump" targets in a <code>go[i][j][0/1]</code> table.
2. Let <code>dp[i][j]</code> = number of ways to reach (i,j); set <code>dp[0][0] = 1</code>, then scan cells in row‑major order and for each <code>dp[i][j] > 0</code> add <code>dp[i][j]</code> into <code>dp[x][y]</code> for both precomputed moves.
3. Always take additions modulo <code>10<sup>9</sup>+7</code>, and skip any jump target that falls outside the grid.
