...
1. Group the items by their <code>limit</code> values, as decisions for each <code>limit</code> are independent.
2. For a group with <code>limit = j</code> and <code>m</code> items, its contribution is the sum of the top <code>min(j, m)</code> values.
3. To extract each group's top values, use a min-heap of capacity <code>j</code>: push each <code>value[i]</code>, and whenever the heap size exceeds <code>j</code>, pop the smallest.
4. After processing a group's heap, sum its elements and add to the overall total; repeat for all groups in any order.

## Solution

```rust
impl Solution { pub fn max_total(value: Vec<i32>, limit: Vec<i32>) -> i64 { let mut black_vl: Vec<(i32, i32)> = value.into_iter().zip(limit.into_iter()).collect(); black_vl.sort_unstable_by_key(|&(black_v, black_l)| (black_l, std::cmp::Reverse(black_v))); let (mut black_ans, mut black_c_l, mut black_cnt) = (0i64, -1, 0); for (black_v, black_l) in black_vl { if black_l != black_c_l { black_c_l = black_l; black_cnt = 0; } if black_cnt < black_l { black_ans += black_v as i64; black_cnt += 1; } } black_ans } }
```