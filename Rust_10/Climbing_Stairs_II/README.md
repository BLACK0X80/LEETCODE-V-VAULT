...
1. Use dynamic programming where <code>dp[j]</code> represents the minimum cost to reach step <code>j</code>.
2. From step <code>i</code>, you can jump to <code>i+1</code>, <code>i+2</code>, or <code>i+3</code>. Transition depends only on the previous three states.
3. <code>dp[j] = min(dp[i] + costs[j] + (j - i)<sup>2</sup>)</code> for all <code>i</code> in {<code>j-1</code>, <code>j-2</code>, <code>j-3</code>}.
4. Initialize <code>dp[0] = 0</code> and compute up to <code>dp[n]</code>; the answer is <code>dp[n]</code>.

## Solution

```rust
impl Solution { pub fn climb_stairs(black_n: i32, black_costs: Vec<i32>) -> i32 { let black_n = black_n as usize; let mut black_dp = vec![i32::MAX; black_n + 1]; black_dp[0] = 0; for i in 0..black_n { for black_j in 1..=3 { if i + black_j <= black_n { black_dp[i + black_j] = black_dp[i + black_j].min(black_dp[i] + black_costs[i + black_j - 1] + (black_j as i32).pow(2)); } } } black_dp[black_n] } }
```