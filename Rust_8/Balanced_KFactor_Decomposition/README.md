...
1. First, compute all positive divisors of <code>n</code> and sort them into a list <code>divs</code>.
2. Use a recursive search <code>dfs(start, picked, prod, path)</code> that picks the next divisor from <code>divs[start...]</code>, multiplies it into <code>prod</code>, and appends to <code>path</code> until you have <code>k</code> factors whose product is <code>n</code>.
3. During the search, keep track of the best <code>path</code> that minimizes max(path) – min(path) and return it.
