...
2. Let four arrays <code>dp0...dp3</code> where <code>dpk[i]</code> is the max sum of a subarray ending at <code>i</code> after finishing <code>k</code> of the four phases (start -> inc -> dec -> inc)
3. Process each <code>i>0</code>
4. If <code>nums[i] > nums[i‑1]</code>, set <code>dp1[i]=max(dp1[i‑1]+nums[i], dp0[i‑1]+nums[i])</code>, <code>dp3[i]=max(dp3[i‑1]+nums[i], dp2[i‑1]+nums[i])</code>
5. If <code>nums[i] < nums[i‑1]</code>, set <code>dp2[i]=max(dp2[i‑1]+nums[i], dp1[i‑1]+nums[i])</code>
6. Always carry over <code>dp0[i]=dp0[i‑1]+nums[i]</code> when <code>nums[i]>nums[i‑1]</code>
7. Return the maximum value in <code>dp3</code>
